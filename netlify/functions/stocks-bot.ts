...
// Paper-trading bot engine (Stocks tab). Runs every 15 minutes; the market-
// hours gate below means real strategy work only happens Mon-Fri 9:30am-4pm
// ET, and the daily-summary block only fires once, at 4:15pm ET. Same
// store-clock caveat as the other Scheduled Functions: this runs on
// Netlify's UTC clock, so all "now" comparisons go through nowInTimeZone.
const MODEL = 'claude-opus-5';

// ── Hard-coded risk rules — the v1 UI can display these but not loosen
// them (per the build spec). Tightening would mean editing these constants.
...
function nowInTimeZone(timeZone: string): Date {
  ...
  const d = new Date(Number(map.year), Number(map.month) - 1, Number(map.day), Number(map.hour), Number(map.minute), Number(map.second));
  return d;
}
...
// Standard EMA series: seeds on the simple average of the first `period`
// closes, then applies the smoothing constant for every bar after.
...
async function reconcileClosedPositions(env: Env, userId: string, keys: AlpacaKeys, livePositions: AlpacaPosition[]) {
...
async function logSignal(env: Env, userId: string, ticker: string, signalType: 'entry' | 'exit' | 'blocked', reason: string, actedOn: boolean) {
...
async function runStrategyForUser(env: Env, userId: string, config: { watchlist: string[]; halted_date: string | null }, keys: AlpacaKeys, nowET: Date) {
  ...
  for (const ticker of config.watchlist) {
    ...
    if (openTickers.has(ticker)) continue; // already holding — no pyramiding
    if (haltedToday) {
...
async function generateDailySummary(env: Env, userId: string, today: string, keys: AlpacaKeys | null) {
...
export default async () => {
  ...
  const env: Env = { supabaseUrl, serviceRoleKey, anthropicKey, headers: { apikey: serviceRoleKey, Authorization: `Bearer ${serviceRoleKey}`, 'content-type': 'application/json' } };
  const nowET = nowInTimeZone('America/New_York');
  const today = dateOnly(nowET);

  if (marketOpen(nowET)) {
    const configRes = await fetch(`${supabaseUrl}/rest/v1/bot_config?enabled=eq.true&select=user_id,watchlist,halted_date`, { headers: env.headers });
    ...
    for (const config of configs) {
      ...
      if (!keys) continue;
      try {
        await runStrategyForUser(env, config.user_id, config, keys, nowET);
      } catch (err) {
      ...
      await fetch(`${supabaseUrl}/rest/v1/bot_config?user_id=eq.${config.user_id}`, {
        method: 'PATCH', headers: env.headers, body: JSON.stringify({ last_run_at: new Date().toISOString() }),
      });
    }
  }
  ...
  if (inWindow(nowET, 16, 15)) {
    const allConfigsRes = await fetch(`${supabaseUrl}/rest/v1/bot_config?select=user_id`, { headers: env.headers });
    const allConfigs = (await allConfigsRes.json()) as { user_id: string }[];
    for (const config of allConfigs) {
      const existingRes = await fetch(`${supabaseUrl}/rest/v1/bot_daily_summary?user_id=eq.${config.user_id}&summary_date=eq.${today}&select=id`, { headers: env.headers });
      const existing = (await existingRes.json()) as { id: string }[];
      if (existing.length > 0) continue;
      const keys = await getBrokerKeys(supabaseUrl, serviceRoleKey, config.user_id);
      try {
        await generateDailySummary(env, config.user_id, today, keys);
      } catch (err) {
