...
export type InboxItemKind = 'mail' | 'message';

/** One row in the owner's unified inbox — the thing pinned above every
 *  nav category. Mail from either domain (support_inbox) or a client's
 *  unread message lands here, newest first. Tickets used to be merged in
 *  here too but now have their own dedicated widget (useOwnerTickets) so
 *  a filed ticket reads as its own category instead of being buried
 *  among plain mail. */
export interface InboxItem {
  ...
  at: string;
  unread: boolean;
  /** Set for a message so opening it can land on that client. */
...
export function useOwnerInbox() {
  ...
  const load = useCallback(async () => {
    const [mail, msgs] = await Promise.all([
