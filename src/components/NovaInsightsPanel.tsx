...
const cardStyle: CSSProperties = { background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '18px 20px', maxWidth: 720 };
const buttonStyle = (disabled: boolean): CSSProperties => ({
  display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 'var(--radius-pill)',
  border: '1px solid var(--text)', color: disabled ? 'var(--text-tertiary)' : 'var(--text)', borderColor: disabled ? 'var(--border)' : 'var(--text)',
  fontSize: 'var(--text-body-sm)', fontWeight: 500, cursor: disabled ? 'default' : 'pointer',
});
...
export default function NovaInsightsPanel({
  ...
  return (
    <div style={{ marginTop: 32, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ fontSize: 'var(--text-label)', fontWeight: 600, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontSize: 'var(--text-body)', fontWeight: 600, color: 'var(--text)' }}>Daily macro target</div>
          <span style={{ fontSize: 'var(--text-small)', color: 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setShowTargetForm((v) => !v)}>
        ...
        {!nutritionTarget && !showTargetForm && (
          <div style={{ fontSize: 'var(--text-body-sm)', color: 'var(--text-tertiary)', marginTop: 8 }}>No target set — meal suggestions will use general judgment instead of hitting specific numbers.</div>
        )}
        {showTargetForm && (
      ...
      <div style={cardStyle}>
        ...
        {latestInsight && (
          <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div>
              <div style={{ fontSize: 'var(--text-caption)', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: 0.4 }}>Nutrient gaps</div>
              <div style={{ fontSize: 'var(--text-body)', color: 'var(--text-quaternary)', marginTop: 3, lineHeight: 1.5 }}>{latestInsight.nutrient_gaps}</div>
            </div>
            <div>
              <div style={{ fontSize: 'var(--text-caption)', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: 0.4 }}>Meal timing</div>
              <div style={{ fontSize: 'var(--text-body)', color: 'var(--text-quaternary)', marginTop: 3, lineHeight: 1.5 }}>{latestInsight.timing_pattern}</div>
            </div>
            <div>
              <div style={{ fontSize: 'var(--text-caption)', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: 0.4 }}>Symptom patterns</div>
              <div style={{ fontSize: 'var(--text-body)', color: 'var(--text-quaternary)', marginTop: 3, lineHeight: 1.5 }}>{latestInsight.symptom_correlations}</div>
            </div>
            <div style={{ fontSize: 'var(--text-tiny)', color: 'var(--text-tertiary)' }}>{latestInsight.window_start} → {latestInsight.window_end}</div>
          </div>
        )}
      </div>
      ...
      {/* Symptom logging */}
      <div style={cardStyle}>
        <div style={{ fontSize: 'var(--text-body)', fontWeight: 600, color: 'var(--text)' }}>Log a symptom</div>
        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {symptomLogs.length > 0 && (
          <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {symptomLogs.slice(0, 5).map((s) => (
              <div key={s.id} style={{ fontSize: 'var(--text-small)', color: 'var(--text-secondary)' }}>
