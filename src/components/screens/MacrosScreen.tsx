...
const statCardStyle: CSSProperties = { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '16px 20px', minWidth: 120 };
const actionPillStyle = (active: boolean): CSSProperties => ({
  ...
  fontSize: 'var(--text-body)', fontWeight: 600, cursor: active ? 'default' : 'pointer',
});
    ...
    <div>
      ...
      {savedMeals.length > 0 && (
        <div style={{ marginTop: 22 }}>
          <div style={{ fontSize: 'var(--text-small)', color: 'var(--text-secondary)', marginBottom: 8 }}>Favorites — tap to log</div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
            {savedMeals.map((sm) => (
              <div
                key={sm.id}
                style={{ flex: '0 0 auto', display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', borderRadius: 'var(--radius-pill)', border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer', whiteSpace: 'nowrap' }}
                onClick={() => logFromSavedMeal(sm)}
              >
                <Icon name="star" size={13} color="var(--text-secondary)" />
                <span style={{ fontSize: 'var(--text-body-sm)', color: 'var(--text)' }}>{sm.name}</span>
                {sm.calories != null && <span style={{ fontSize: 'var(--text-caption)', color: 'var(--text-tertiary)' }}>{sm.calories} cal</span>}
                <span style={{ fontSize: 'var(--text-small)', color: 'var(--text-tertiary)', marginLeft: 4 }} onClick={(e) => { e.stopPropagation(); removeSavedMeal(sm.id); }}>✕</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 16, flexWrap: 'wrap', maxWidth: 720, alignItems: 'center' }}>
        ...
        <span style={{ fontSize: 'var(--text-small)', color: 'var(--text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }} onClick={saveCurrentAsFavorite}>
          <Icon name="star" size={13} color="var(--text-secondary)" /> Save as favorite
        </span>
      </div>

      <div style={{ marginTop: 18, display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', overflow: 'hidden', maxWidth: 720 }}>
        {todayMeals.map((m) => (
          <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '14px 20px', borderBottom: '1px solid var(--surface-3)', background: 'var(--surface-2)' }}>
            <div>
              <div style={{ fontSize: 'var(--text-label)', fontWeight: 600, color: 'var(--text)' }}>
                {m.meal_type[0].toUpperCase() + m.meal_type.slice(1)}{m.restaurant_name ? ` · ${m.restaurant_name}` : ''}
                {m.log_method !== 'manual' && (
                  <span style={{ fontSize: 'var(--text-micro)', color: 'var(--text-tertiary)', fontWeight: 500, marginLeft: 8, textTransform: 'uppercase' }}>{m.log_method}</span>
                )}
              </div>
            ...
            <span style={{ fontSize: 'var(--text-body)', color: 'var(--text-tertiary)', cursor: 'pointer' }} onClick={() => removeMeal(m.id)}>Remove</span>
          </div>
        ))}
        {!loading && todayMeals.length === 0 && (
          <div style={{ padding: '18px', fontSize: 'var(--text-body)', color: 'var(--text-tertiary)', background: 'var(--surface-2)' }}>No meals logged today.</div>
        )}
      </div>

      <div
        style={{ marginTop: 28, fontSize: 'var(--text-body)', color: 'var(--text-secondary)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }}
      ...
      {showFastFood && (
        <div style={{ marginTop: 14, maxWidth: 720 }}>
          {fastFood.length === 0 && (
            <div
              style={{ display: 'inline-flex', alignItems: 'center', padding: '8px 16px', borderRadius: 'var(--radius-pill)', border: '1px solid var(--text)', color: 'var(--text)', fontSize: 'var(--text-body-sm)', cursor: seedingFastFood ? 'default' : 'pointer', marginBottom: 12, opacity: seedingFastFood ? 0.6 : 1 }}
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
              {(['high-protein-low-cal', 'best-value', 'low-carb'] as const).map((tag) => (
                <span
                  key={tag}
                  style={{
                    fontSize: 'var(--text-caption)', padding: '5px 12px', borderRadius: 'var(--radius-pill)', cursor: 'pointer',
          ...
          <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', overflow: 'hidden' }}>
            {visibleFastFood.map((f) => (
              <div key={f.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 18px', borderBottom: '1px solid var(--surface-3)', background: 'var(--surface-2)' }}>
                <div>
                  <span style={{ fontSize: 'var(--text-body)', color: 'var(--text-quaternary)' }}>{f.restaurant_name} — {f.item_name}{f.calories ? ` (${f.calories} cal)` : ''}</span>
                  {f.goal_tags?.length > 0 && (
                    <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                      {f.goal_tags.map((t) => (
                        <span key={t} style={{ fontSize: 'var(--text-micro)', color: 'var(--text-tertiary)' }}>{GOAL_TAG_LABELS[t] ?? t}</span>
                      ))}
                    </div>
                  )}
                </div>
                <span style={{ fontSize: 'var(--text-small)', color: 'var(--text-tertiary)', cursor: 'pointer' }} onClick={() => removeFastFoodOption(f.id)}>Remove</span>
              </div>
            ))}
            {visibleFastFood.length === 0 && (
              <div style={{ padding: '14px 18px', fontSize: 'var(--text-body-sm)', color: 'var(--text-tertiary)', background: 'var(--surface-2)' }}>Nothing here yet — add your go-to orders, or load the starter list above.</div>
            )}
          </div>
        </div>
      )}
