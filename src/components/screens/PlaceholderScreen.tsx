...
export default function PlaceholderScreen({ isMobile, label, note }: Props) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', paddingTop: isMobile ? 60 : 80 }}>
      ...
      <div style={{ fontSize: 20, fontWeight: 600, color: 'var(--text)', marginBottom: 6 }}>{label}</div>
      <div style={{ fontSize: 14, color: 'var(--text-secondary)', marginTop: 6, maxWidth: 420 }}>{note || 'This section is coming soon.'}</div>
    </div>
  );
}
