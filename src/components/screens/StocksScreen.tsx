...
const pillButton = (variant: 'solid' | 'outline', danger?: boolean): CSSProperties => ({
  ...
  background: variant === 'solid' ? (danger ? RED : '#F5F6F7') : 'transparent',
  color: variant === 'solid' ? '#0A0B0D' : danger ? RED : '#F5F6F7',
  border: variant === 'outline' ? `1px solid ${danger ? RED : '#F5F6F7'}` : 'none',
});
...
function pnlColor(n: number): string {
  return n > 0 ? GREEN : n < 0 ? RED : '#8A8F98';
}
  ...
  return (
    <div style={{ ...cardStyle, marginTop: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }} onClick={() => setOpen(!open)}>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#F5F6F7' }}>How to run this bot</div>
        <div style={{ fontSize: 12.5, color: '#8A8F98' }}>{doneCount}/{WALKTHROUGH_STEPS.length} · {open ? 'Hide' : 'Show'}</div>
      </div>
      {open && (
          ...
          {WALKTHROUGH_STEPS.map((step, i) => (
            <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer' }} onClick={() => toggle(i)}>
              <div style={{
                ...
                border: `1px solid ${checked[i] ? GOLD : '#2b2f36'}`, background: checked[i] ? GOLD : 'transparent',
                display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#0A0B0D', fontWeight: 700,
              }}>
                {checked[i] ? '✓' : ''}
              </div>
              <div style={{ fontSize: 13, color: checked[i] ? '#565b64' : '#c8cad0', textDecoration: checked[i] ? 'line-through' : 'none', lineHeight: 1.5 }}>
                {step}
              </div>
            </div>
          ))}
...
function Sparkline({ points }: { points: number[] }) {
  if (points.length < 2) return <div style={{ fontSize: 12.5, color: '#565b64', padding: '30px 0' }}>Not enough days yet — check back after a few trading days.</div>;
    ...
    <div style={{ marginTop: 20 }}>
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
        <div style={{ ...cardStyle, minWidth: 160 }}>
          <div style={{ ...mono, fontSize: 26, fontWeight: 600, color: accountLoading ? '#565b64' : pnlColor(account.dailyPl) }}>
            {accountLoading ? '—' : money(account.dailyPl)}
          </div>
          <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 4 }}>Net P&L today {accountLoading ? '' : `(${pct(account.dailyPlPct)})`}</div>
        </div>
        <div style={{ ...cardStyle, minWidth: 160 }}>
          <div style={{ ...mono, fontSize: 26, fontWeight: 600, color: '#F5F6F7' }}>{accountLoading ? '—' : money(account.equity)}</div>
          <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 4 }}>Paper account equity</div>
        </div>
        <div style={{ ...cardStyle, minWidth: 160 }}>
          <div style={{ ...mono, fontSize: 26, fontWeight: 600, color: '#F5F6F7' }}>{account.positions.length}</div>
          <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 4 }}>Open positions</div>
        </div>
      </div>

      <div style={{ fontSize: 13, fontWeight: 600, color: '#F5F6F7', marginTop: 24, marginBottom: 10 }}>Open positions</div>
      <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid #22262B', borderRadius: 14, overflow: 'hidden' }}>
        {account.positions.map((p) => (
          <div key={p.symbol} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '14px 20px', borderBottom: '1px solid #1c1e23', background: '#101114' }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#F5F6F7' }}>{p.symbol}</div>
              <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 2 }}>{p.qty} sh · avg {money(p.avg_entry_price)} · now {money(p.current_price)}</div>
            </div>
        ...
        {!accountLoading && account.positions.length === 0 && (
          <div style={{ padding: 18, fontSize: 13, color: '#565b64', background: '#101114' }}>No open positions right now.</div>
        )}
      </div>

      <div style={{ fontSize: 13, fontWeight: 600, color: '#F5F6F7', marginTop: 24, marginBottom: 10 }}>
        Today's signals <span style={{ color: '#565b64', fontWeight: 400 }}>— including blocked ones, so you can see why the bot did (or didn't) act</span>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid #22262B', borderRadius: 14, overflow: 'hidden' }}>
        {todaySignals.map((s) => <SignalRow key={s.id} signal={s} />)}
        {todaySignals.length === 0 && <div style={{ padding: 18, fontSize: 13, color: '#565b64', background: '#101114' }}>No signals yet today.</div>}
      </div>
      {openTrades.length === 0 && account.positions.length === 0 && (
        <div style={{ fontSize: 12, color: '#565b64', marginTop: 10 }}>No trades open — the bot is watching the watchlist for a setup.</div>
      )}
    </div>
  );
}
...
function SignalRow({ signal }: { signal: BotSignal }) {
  const color = signal.signal_type === 'entry' ? GREEN : signal.signal_type === 'exit' ? GOLD : '#565b64';
  const label = signal.signal_type === 'entry' ? 'Entered' : signal.signal_type === 'exit' ? 'Exited' : 'Blocked';
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 20px', borderBottom: '1px solid #1c1e23', background: '#101114' }}>
      <div style={{ fontSize: 11, fontWeight: 700, color, textTransform: 'uppercase', letterSpacing: 0.4, minWidth: 56, marginTop: 2 }}>{label}</div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13.5, fontWeight: 600, color: '#F5F6F7' }}>{signal.ticker === '*' ? 'Bot-wide' : signal.ticker}</div>
        <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 2 }}>{signal.reason}</div>
      </div>
      <div style={{ fontSize: 11.5, color: '#565b64', whiteSpace: 'nowrap' }}>{new Date(signal.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
    </div>
  );
}
...
function PerformancePanel({ trades, dailySummaries }: ReturnType<typeof useStocksBot>) {
  ...
  return (
    <div style={{ marginTop: 20 }}>
      <div style={cardStyle}>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#F5F6F7', marginBottom: 12 }}>Equity curve since start</div>
        <Sparkline points={equityPoints} />
      </div>
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: 16 }}>
        <div style={{ ...cardStyle, minWidth: 140 }}>
          <div style={{ ...mono, fontSize: 22, fontWeight: 600, color: '#F5F6F7' }}>{winRate == null ? '—' : `${winRate.toFixed(0)}%`}</div>
          <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 4 }}>Win rate</div>
        </div>
        <div style={{ ...cardStyle, minWidth: 140 }}>
          <div style={{ ...mono, fontSize: 22, fontWeight: 600, color: '#F5F6F7' }}>{closed.length}</div>
          <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 4 }}>Total closed trades</div>
        </div>
        <div style={{ ...cardStyle, minWidth: 140 }}>
          <div style={{ ...mono, fontSize: 22, fontWeight: 600, color: GREEN }}>{money(avgWin)}</div>
          <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 4 }}>Average win</div>
        </div>
        <div style={{ ...cardStyle, minWidth: 140 }}>
          <div style={{ ...mono, fontSize: 22, fontWeight: 600, color: RED }}>{money(avgLoss)}</div>
          <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 4 }}>Average loss</div>
        </div>
        <div style={{ ...cardStyle, minWidth: 140 }}>
          <div style={{ ...mono, fontSize: 22, fontWeight: 600, color: RED }}>{maxDrawdown.toFixed(1)}%</div>
          <div style={{ fontSize: 12.5, color: '#8A8F98', marginTop: 4 }}>Max drawdown</div>
        </div>
      </div>
  ...
  return (
    <div style={{ marginTop: 20 }}>
      ...
      <div style={{ overflowX: 'auto' }}>
        <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid #22262B', borderRadius: 14, overflow: 'hidden', minWidth: 560 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '80px 60px 70px 90px 90px 90px 90px 1fr', gap: 8, padding: '10px 20px', background: '#1a1c21', fontSize: 11, color: '#565b64', textTransform: 'uppercase', letterSpacing: 0.3 }}>
            <div>Ticker</div><div>Side</div><div>Qty</div><div>Entry</div><div>Exit</div><div>Stop</div><div>P&L</div><div>Opened</div>
          </div>
          {filtered.map((t) => (
            <div key={t.id} style={{ display: 'grid', gridTemplateColumns: '80px 60px 70px 90px 90px 90px 90px 1fr', gap: 8, padding: '12px 20px', borderTop: '1px solid #1c1e23', background: '#101114', fontSize: 13, alignItems: 'center' }}>
              <div style={{ fontWeight: 600, color: '#F5F6F7' }}>{t.ticker}</div>
              <div style={{ color: '#8A8F98' }}>{t.side}</div>
              ...
              <div style={{ ...mono, color: '#8A8F98' }}>{t.stop_loss_price != null ? money(t.stop_loss_price) : '—'}</div>
              <div style={{ ...mono, color: t.pnl != null ? pnlColor(t.pnl) : '#565b64' }}>{t.pnl != null ? money(t.pnl) : t.status === 'open' ? 'open' : '—'}</div>
              <div style={{ color: '#565b64', fontSize: 12 }}>{new Date(t.opened_at).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</div>
            </div>
          ))}
          {filtered.length === 0 && <div style={{ padding: 18, fontSize: 13, color: '#565b64', background: '#101114' }}>No trades yet.</div>}
        </div>
      </div>
    </div>
  );
...
function SettingsPanel(bot: ReturnType<typeof useStocksBot>) {
  ...
  return (
    <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 640 }}>
      <div style={cardStyle}>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#F5F6F7', marginBottom: 4 }}>Broker keys — Alpaca (paper)</div>
        <div style={{ fontSize: 12.5, color: '#8A8F98', marginBottom: 12 }}>
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#F5F6F7', marginBottom: 8 }}>Watchlist <span style={{ color: '#565b64', fontWeight: 400 }}>(5 max)</span></div>
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#F5F6F7', marginBottom: 10 }}>Risk rules <span style={{ color: '#565b64', fontWeight: 400 }}>(view only — hard limits, can't be loosened here)</span></div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {RISK_RULES.map((r, i) => (
            <div key={i} style={{ fontSize: 13, color: '#c8cad0', display: 'flex', gap: 8 }}>
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#F5F6F7', marginBottom: 10 }}>Strategy</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
    ...
    <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
      {account.news.map((n, i) => (
        <a key={i} href={n.url} target="_blank" rel="noreferrer" style={{ ...cardStyle, display: 'block', textDecoration: 'none' }}>
          <div style={{ fontSize: 13.5, fontWeight: 600, color: '#F5F6F7', lineHeight: 1.4 }}>{n.headline}</div>
          <div style={{ fontSize: 12, color: '#565b64', marginTop: 6 }}>
      ...
      {!accountLoading && account.news.length === 0 && (
        <div style={cardStyle}>
          <div style={{ fontSize: 13, color: '#565b64' }}>{account.connected ? 'No recent headlines for your watchlist.' : 'Connect your Alpaca keys to see market news for your watchlist.'}</div>
        </div>
      )}
    </div>
  );
      ...
      <div style={{ ...cardStyle, marginTop: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: config.enabled ? GREEN : '#565b64' }} />
            <div style={{ fontSize: 13.5, fontWeight: 600, color: '#F5F6F7' }}>{config.enabled ? 'Running' : 'Paused'}</div>
          </div>
          {halted && (
          ...
          <div style={{ fontSize: 12, color: '#565b64' }}>Last run: {timeAgo(config.last_run_at)}</div>
        </div>
