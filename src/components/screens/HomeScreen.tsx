 ...
 *  one. */
export default function HomeScreen({ homeHeadStyle, homeSubStyle, statCards, onOpenNova, assistantName, onNavigate }: Props) {
  ...
  return (
    <div>
      <div style={homeHeadStyle}>{greeting()}{assistantName ? '' : ''}.</div>
      <div style={homeSubStyle}>{nudgeSummary}</div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 11, marginTop: 24 }}>
        {cards.map((card, i) => {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.05fr) minmax(0, 0.95fr) minmax(280px, 340px)', gap: 12, marginTop: 18 }}>
        ...
        <div style={{ ...cardShell, background: 'var(--accent-soft)', border: 'none', padding: 20, gap: 14 }}>
          ...
          <div
            onClick={onOpenNova}
            style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 15px', borderRadius: 12, background: 'var(--text)', color: 'var(--bg)', fontSize: 13.5, fontWeight: 600, cursor: 'pointer' }}
          >
