...
export default function LeadFlowHistory() {
  ...
  return (
    <div>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: '1rem', boxShadow: '0 1px 3px rgba(0,0,0,0.07)' }}>
        {history.map((item, i) => (
          <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '1rem 0', borderBottom: i < history.length - 1 ? '1px solid #f3f4f6' : 'none', gap: 12, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: '#6b7280' }}>{item.industry} · {new Date(item.created_at).toLocaleString()}</div>
                <div style={{ fontSize: 14, color: '#374151', marginTop: 4 }}>{item.note}</div>
              </div>
            </div>
            {item.tag && (
              <span style={{ background: (TAG_COLORS[item.tag] ?? '#6b7280') + '22', color: TAG_COLORS[item.tag] ?? '#6b7280', borderRadius: 20, padding: '4px 12px', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>
