  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
        <div style={{ width: 220 }}>
          {CONTACTS.map((c) => (
            <div key={c.name} onClick={() => setSelected(c)} style={{ padding: '12px 16px', borderRadius: 10, marginBottom: 8, cursor: 'pointer', background: selected.name === c.name ? '#f0fdf4' : '#fff', border: selected.name === c.name ? '1px solid #bbf7d0' : '1px solid #e5e7eb' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div><strong style={{ fontSize: 14 }}>{c.name}</strong><div style={{ fontSize: 12, color: '#6b7280' }}>{c.role}</div></div>
              </div>
            </div>
          ))}
        </div>
        <div style={{ flex: 1, minWidth: 280, background: '#fff', borderRadius: 12, padding: '1.5rem', boxShadow: '0 1px 3px rgba(0,0,0,0.07)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: '1rem', paddingBottom: '1rem', borderBottom: '1px solid #f3f4f6' }}>
            ...
            <div><strong>{selected.name}</strong><div style={{ fontSize: 13, color: '#6b7280' }}>{selected.role}</div></div>
          </div>
          <div style={{ marginBottom: '1rem' }}>
            ...
            {messages.map((n) => (
              <div key={n.id} style={{ padding: '10px 0', borderBottom: '1px solid #f3f4f6' }}>
                ...
                <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 2 }}>{new Date(n.created_at).toLocaleString()}</div>
              </div>
            ))}
          </div>
          <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder={`Quick note or message to ${selected.name}...`} style={{ width: '100%', padding: 12, borderRadius: 8, border: '1px solid #e5e7eb', resize: 'none', height: 80, boxSizing: 'border-box', marginBottom: 8 }} />
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button onClick={save} style={{ background: GREEN, color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', cursor: 'pointer', fontWeight: 600 }}>✉️ Save Note</button>
          </div>
        </div>
      </div>
    </div>
