...
function CategoryForm({ onAdd }: { onAdd: (name: string, amount: number) => Promise<void> }) {
  ...
  if (!open) return <div style={{ fontSize: 12.5, color: 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setOpen(true)}>+ Add category</div>;
  return (
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
      ...
      <div style={{ padding: '8px 16px', borderRadius: 999, background: 'var(--text)', color: 'var(--bg)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }} onClick={submit}>Save</div>
      <div style={{ fontSize: 12, color: 'var(--text-tertiary)', cursor: 'pointer' }} onClick={() => setOpen(false)}>Cancel</div>
    </div>
  );
}
...
function TransactionForm({ categories, onAdd }: { categories: { id: string; name: string }[]; onAdd: (type: BudgetType, amount: number, description: string, categoryId: string | null, occurredOn: string) => Promise<void> }) {
  ...
  return (
    <div style={{ ...cardStyle, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
      ...
      <div style={{ padding: '9px 18px', borderRadius: 999, background: 'var(--text)', color: 'var(--bg)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }} onClick={submit}>Log it</div>
    </div>
  );
}
...
function RecurringForm({ categories, onAdd }: { categories: { id: string; name: string }[]; onAdd: (input: { type: BudgetType; name: string; amount: number; cadence: Cadence; category_id: string | null; next_occurrence: string }) => Promise<void> }) {
  ...
  if (!open) return <div style={{ fontSize: 12.5, color: 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setOpen(true)}>+ Add recurring item</div>;
  return (
    <div style={{ ...cardStyle, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
      <select style={{ ...inputStyle, width: 100 }} value={type} onChange={(e) => setType(e.target.value as BudgetType)}>
        <option value="expense">Expense</option>
...
function SubscriptionForm({ onAdd }: { onAdd: (input: { name: string; cost: number; billing_cycle: BillingCycle; renewal_date: string; category: string | null }) => Promise<void> }) {
  ...
  if (!open) return <div style={{ fontSize: 12.5, color: 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setOpen(true)}>+ Add subscription</div>;
  return (
    <div style={{ ...cardStyle, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 24 }}>
        <span style={{ fontSize: 16, cursor: 'pointer', color: 'var(--text-secondary)' }} onClick={() => setMonthKey(shiftMonthKey(monthKey, -1))}>&larr;</span>
        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)', minWidth: 140, textAlign: 'center' }}>{monthLabel(monthKey)}</div>
        <span style={{ fontSize: 16, cursor: isCurrentMonth ? 'default' : 'pointer', color: isCurrentMonth ? '#33363c' : 'var(--text-secondary)' }} onClick={() => !isCurrentMonth && setMonthKey(shiftMonthKey(monthKey, 1))}>&rarr;</span>
      </div>
        ...
        {summary.byCategory.map(({ category, allocated, spent, remaining }) => {
          ...
          return (
            <div key={category.id} style={cardStyle}>
              ...
              <div style={{ height: 6, background: 'var(--border)', borderRadius: 999, marginTop: 10, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${pct}%`, background: over ? '#c47a7a' : 'var(--text)', borderRadius: 999 }} />
              </div>
              <div style={{ fontSize: 11.5, color: over ? '#c47a7a' : 'var(--text-tertiary)', marginTop: 6 }}>
                {over ? `${money(Math.abs(remaining))} over` : `${money(remaining)} remaining`}
              </div>
            </div>
          );
        })}
        {summary.byCategory.length === 0 && <div style={{ fontSize: 12.5, color: 'var(--text-tertiary)' }}>No categories yet.</div>}
      ...
      />
      <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden', marginTop: 12 }}>
        {budgeting.transactions.filter((t) => t.occurred_on.slice(0, 7) === monthKey).slice(0, 25).map((t) => (
          <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', borderBottom: '1px solid var(--surface-3)', fontSize: 12.5 }}>
        ...
        {budgeting.paidInvoices.filter((i) => i.paid_at.slice(0, 7) === monthKey).map((inv) => (
          <div key={inv.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', borderBottom: '1px solid var(--surface-3)', fontSize: 12.5 }}>
        ...
        {budgeting.transactions.filter((t) => t.occurred_on.slice(0, 7) === monthKey).length === 0 && budgeting.paidInvoices.filter((i) => i.paid_at.slice(0, 7) === monthKey).length === 0 && (
          <div style={{ padding: 16, fontSize: 12.5, color: 'var(--text-tertiary)' }}>Nothing logged for this month yet.</div>
        )}
      </div>
        ...
        {budgeting.recurring.map((r) => (
          <div key={r.id} style={{ ...cardStyle, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--text-secondary)', marginTop: 2 }}>{CADENCE_LABEL[r.cadence]} · next {new Date(r.next_occurrence + 'T00:00:00').toLocaleDateString()}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontWeight: 600, color: r.type === 'income' ? '#8fae8f' : '#c47a7a' }}>{r.type === 'income' ? '+' : '-'}{money(r.amount)}</span>
              <span style={{ color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: 12 }} onClick={() => budgeting.removeRecurring(r.id)}>Remove</span>
            </div>
          </div>
        ))}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
        {history.map((h) => (
          <div key={h.monthKey} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 16px', borderBottom: '1px solid var(--surface-3)', fontSize: 12.5 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 16 }}>
        {subs.subscriptions.map((s) => (
          <div key={s.id} style={{ ...cardStyle, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{s.name}{s.category ? <span style={{ color: 'var(--text-tertiary)', fontWeight: 400 }}> · {s.category}</span> : null}</div>
              <div style={{ fontSize: 11.5, color: 'var(--text-secondary)', marginTop: 2 }}>{CYCLE_LABEL[s.billing_cycle]} · renews {new Date(s.renewal_date + 'T00:00:00').toLocaleDateString()}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontWeight: 600, color: 'var(--text)' }}>{money(s.cost)}</span>
              {!isStale(s) && <span style={{ color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: 12 }} onClick={() => subs.markUsed(s.id)}>Mark used</span>}
              <span style={{ color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: 12 }} onClick={() => subs.removeSubscription(s.id)}>Remove</span>
            </div>
          </div>
        ))}
        {subs.subscriptions.length === 0 && <div style={{ fontSize: 12.5, color: 'var(--text-tertiary)' }}>No subscriptions tracked yet.</div>}
        <SubscriptionForm onAdd={subs.addSubscription} />
      </div>

      <div style={{ ...cardStyle, marginTop: 20, maxWidth: 480 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 4 }}>Mastermind savings comparison</div>
        <div style={{ fontSize: 11.5, color: 'var(--text-secondary)', marginBottom: 12 }}>What you'd pay for what Mastermind consolidates, vs. what Mastermind costs.</div>
