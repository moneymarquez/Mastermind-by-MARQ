  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 20, flexWrap: 'wrap' }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '16px 20px', minWidth: 140 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 28, fontWeight: 600, color: 'var(--text)' }}>{loading ? '—' : weekCount}</div>
          <div style={{ fontSize: 'var(--text-body-sm)', color: 'var(--text-secondary)', marginTop: 4 }}>Workouts this week</div>
        </div>
      </div>
      ...
      {tab === 'log' && (
        <div style={{ marginTop: 20 }}>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <div
              style={{ display: 'flex', alignItems: 'center', padding: '10px 18px', borderRadius: 'var(--radius-pill)', background: generating ? 'var(--border)' : 'var(--text)', color: generating ? 'var(--text-secondary)' : 'var(--bg)', fontSize: 'var(--text-body)', fontWeight: 600, cursor: generating ? 'default' : 'pointer' }}
              onClick={() => !generating && generate('workout')}
            >
              {generating === 'workout' ? 'Generating…' : '✨ AI workout plan'}
            </div>
            <div
              style={{ display: 'flex', alignItems: 'center', padding: '10px 18px', borderRadius: 'var(--radius-pill)', border: '1px solid var(--text)', color: generating ? 'var(--text-tertiary)' : 'var(--text)', fontSize: 'var(--text-body)', fontWeight: 500, cursor: generating ? 'default' : 'pointer' }}
          ...
          {(['workout', 'diet'] as FitnessPlanKind[]).map((kind) => {
            ...
            if (!latest) return null;
            return (
              <div key={kind} style={{ marginTop: 12, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: 22, maxWidth: 640 }}>
                <div style={{ fontSize: 'var(--text-body-lg)', color: 'var(--text)', whiteSpace: 'pre-wrap', lineHeight: 1.7, fontFamily: 'var(--font-mono)' }}>{latest.plan_text}</div>
              </div>
            );
          })}
          ...
          <div style={{ marginTop: 18, display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', overflow: 'hidden', maxWidth: 640 }}>
            {workouts.map((w) => (
              <div key={w.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '14px 20px', borderBottom: '1px solid var(--surface-3)', background: 'var(--surface-2)' }}>
                <div>
                  <div style={{ fontSize: 'var(--text-label)', fontWeight: 600, color: 'var(--text)' }}>{w.workout_type}</div>
                ...
                <span style={{ fontSize: 'var(--text-body)', color: 'var(--text-tertiary)', cursor: 'pointer' }} onClick={() => removeWorkout(w.id)}>Remove</span>
              </div>
            ))}
            {!loading && workouts.length === 0 && (
              <div style={{ padding: '18px', fontSize: 'var(--text-body)', color: 'var(--text-tertiary)', background: 'var(--surface-2)' }}>No workouts logged yet.</div>
            )}
          </div>
        </div>
      )}
