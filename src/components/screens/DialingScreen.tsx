...
const cardStyle: CSSProperties = { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 22 };
const outcomeBtn: CSSProperties = {
  display: 'inline-flex', alignItems: 'center', padding: '7px 12px', borderRadius: 999,
  border: '1px solid var(--border)', color: 'var(--text-quaternary)', fontSize: 11.5, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap',
};
const primaryBtn: CSSProperties = {
  display: 'inline-flex', alignItems: 'center', padding: '10px 18px', borderRadius: 999,
  background: 'var(--text)', color: 'var(--bg)', fontSize: 13, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap',
};
  ...
  return (
    <div>
      ...
      <div style={{ ...cardStyle, marginTop: 24, maxWidth: 640 }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Current Pitch</div>
        <textarea
          style={{
            width: '100%', minHeight: 110, marginTop: 12, background: 'var(--surface-4)', border: '1px solid var(--border-2)', borderRadius: 8,
            padding: '12px 14px', color: 'var(--text)', fontSize: 13.5, lineHeight: 1.6, outline: 'none', resize: 'vertical', fontFamily: 'inherit',
          }}
        ...
        />
        <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 6 }}>Saves automatically when you click away.</div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 16, marginTop: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ ...cardStyle, flex: '1 1 260px' }}>
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 40, fontWeight: 600, color: 'var(--text)' }}>
            {todayCount} <span style={{ fontSize: 20, color: 'var(--text-tertiary)' }}>/ {DAILY_CALL_GOAL}</span>
          </div>
          <div style={{ height: 8, background: 'var(--border)', borderRadius: 999, marginTop: 14, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${pct}%`, background: 'var(--text)', borderRadius: 999 }} />
          </div>
          <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 8 }}>Calls completed today</div>
        </div>
      ...
      <div style={{ marginTop: 28 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 12 }}>Today's Calls ({activeQueue.length})</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 680 }}>
          {activeQueue.map((c) => {
            const appt = appointmentLabel(c);
            return (
              <div key={c.id} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>{c.name}</div>
                    <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>{c.phone}{appt ? ` · ${appt}` : ''}</div>
                  </div>
                </div>
          ...
          {activeQueue.length === 0 && (
            <div style={{ padding: 18, fontSize: 13, color: 'var(--text-tertiary)', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 12 }}>
      ...
      <div style={{ marginTop: 24, maxWidth: 680 }}>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }} onClick={() => setShowCompleted((v) => !v)}>
          ...
          <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
            {completedToday.map(({ outcome, contact }) => (
              <div key={outcome.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '12px 18px', borderBottom: '1px solid var(--surface-3)', background: 'var(--surface-2)' }}>
                <div>
                  <span style={{ fontSize: 13, color: 'var(--text-quaternary)' }}>{contact?.name ?? 'Unknown contact'}</span>
                  <span style={{ fontSize: 12, color: 'var(--text-secondary)', marginLeft: 8 }}>{CALL_OUTCOME_LABEL[outcome.outcome]} · {loggedAtLabel(outcome.logged_at)}</span>
                </div>
                <span style={{ fontSize: 12, color: 'var(--text-tertiary)', cursor: 'pointer' }} onClick={() => undoOutcome(outcome.id)}>Undo</span>
              </div>
            ))}
            {completedToday.length === 0 && (
              <div style={{ padding: 18, fontSize: 13, color: 'var(--text-tertiary)', background: 'var(--surface-2)' }}>Nothing logged yet today.</div>
            )}
          </div>
        )}
      </div>
      ...
      <div style={{ marginTop: 18, maxWidth: 680 }}>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }} onClick={() => setShowHistory((v) => !v)}>
        ...
        {showHistory && (
          <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden' }}>
            {history.map((day) => (
              <div key={day.date} style={{ padding: '12px 18px', borderBottom: '1px solid var(--surface-3)', background: 'var(--surface-2)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{formatDateLabel(day.date)}</span>
                  <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{day.total} calls</span>
                </div>
            ...
            {history.length === 0 && (
              <div style={{ padding: 18, fontSize: 13, color: 'var(--text-tertiary)', background: 'var(--surface-2)' }}>No calls logged yet.</div>
            )}
          </div>
        )}
      </div>
