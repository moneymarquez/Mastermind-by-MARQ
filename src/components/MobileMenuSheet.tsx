 ...
 *  bottom sheet with a drag handle, a Menu header, the same real NavRow[]
 *  Sidebar.tsx renders on desktop, and an account footer. Replaces the old
 *  top-right dropdown NavDrawer/floating Logo, both deleted — Sidebar.tsx
 *  covers desktop, this covers mobile, and nothing else referenced either
 *  component.
 *
 *  No theme toggle here — dark/light is a Settings-only control, not
 *  duplicated in every surface that happens to have room for one. */
export default function MobileMenuSheet({ open, rows, ownerName, isOwner, onClose, onOpenSettings, onOpenTour }: Props) {
  if (!open) return null;
  return (
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div title="Take the product tour" onClick={() => { onOpenTour(); onClose(); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: '50%', border: '1px solid var(--mm-line2)', fontSize: 13, fontWeight: 700, color: 'var(--mm-dim)' }}>?</div>
        </div>
        ...
        <div
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 11, padding: 12, borderRadius: 16, background: 'var(--mm-tile)', cursor: 'pointer' }}
        >
