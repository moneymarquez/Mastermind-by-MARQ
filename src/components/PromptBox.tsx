 ...
 *  iOS in-app browsers). */
export default function PromptBox({ title, hint, text }: Props) {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      />
      <div style={{ fontSize: 'var(--text-tiny)', color: 'var(--text-tertiary)' }}>{text.length.toLocaleString()} characters</div>
    </div>
  );
}
