...
export async function stocksAccount(request: Request, env: StocksEnv): Promise<Response> {
  const user = await requireUser(request, env.VITE_SUPABASE_URL, env.VITE_SUPABASE_ANON_KEY);
  if (user instanceof Response) return user;

  const keys = await getBrokerKeys(env.VITE_SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY, user.id);
  if (!keys) {
  ...
  const configRes = await fetch(`${env.VITE_SUPABASE_URL}/rest/v1/bot_config?user_id=eq.${user.id}&select=watchlist`, {
